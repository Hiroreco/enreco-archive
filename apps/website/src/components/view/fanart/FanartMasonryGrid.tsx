...
const NewsPostItem = ({ post, index }: NewsPostItemProps) => {
    ...
    return (
        <a
            ...
            className="text-sm block border border-foreground/20 rounded-lg p-4 hover:bg-accent/30 transition-colors group !text-foreground !visited:text-foreground mb-4 w-full max-w-full overflow-hidden"
        >
            {/* Header */}
...
const NewsModal = ({ open, onClose }: NewsModalProps) => {
    ...
    useEffect(() => {
        if (currentPage === 1) {
            setMasonryColumns(calculateMasonryLayout(paginatedNews));
            return;
        }
        ...
        setMasonryColumns((prev) => {
            ...
            const newPosts = paginatedNews.slice(startIndex);

            newPosts.forEach((post, index) => {
                const shortestColumn = next.reduce(
                    (min, col, i) => (col.height < next[min].height ? i : min),
                    0,
                );
                ...
                next[shortestColumn].items.push({
                    post,
                    index: startIndex + index,
                });
                next[shortestColumn].height += estimatedHeight;
            });

            return next;
        });
    }, [paginatedNews, currentPage, calculateMasonryLayout]);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent
                showXButton={true}
                className="rounded-lg w-full max-w-[95vw] lg:max-w-[1000px] h-[90vh] flex flex-col p-6"
