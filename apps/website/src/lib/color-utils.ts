...
function calcRelativeLuminance(rgbValues: [number, number, number]) {
    ...
    return (
        calcComponentLuminance(rgbValues[0]) * 0.2126 +
...
export function getContrastedColor(color: string, isDarkMode: boolean) {
    ...
    const linkColorLuminance = calcRelativeLuminance([
        linkColorRgb.values[0],
        linkColorRgb.values[1],
        linkColorRgb.values[2],
    ]);
    const bgColorLuminance = calcRelativeLuminance(
    ...
    const linkColorHsl = colorSpace.rgb.hsl([
        linkColorRgb.values[0],
        linkColorRgb.values[1],
        linkColorRgb.values[2],
    ]);
    ...
    while (
        contrastRatio < TARGET_CONTRAST_RATIO &&
        (linkColorHsl[2] < 100 || linkColorHsl[2] > 0)
    ) {
        if (!isDarkMode) {
