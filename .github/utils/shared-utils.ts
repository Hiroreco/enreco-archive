...
export function countChecklistItems(
  ...
  const thingToCheckfor = type === "submitted" ? "(submitted)" : "- [x]";
...
export function crossOffItem(issueBody: string, item: string): string {
  ...
  let result = issueBody.replace(pattern, `$1[x] ${item}$3 (submitted)`);
    ...
    if (botComment && typeof botComment.body === "string") {
      ...
      if (match) {
        if (stateType === "CHECKLIST_TRACKER") {
          return Buffer.from(match[1], "base64").toString("utf-8") as T;
        } else {
...
export function extractProofreaderAssignments(
  ...
  for (const line of lines) {
    ...
    for (const section of sections) {
      assignments[section] = proofreader;
    }
  }
...
export function extractItemStates(
  issueBody: string,
): Record<string, "checked" | "unchecked" | "submitted"> {
  const itemStates: Record<string, "checked" | "unchecked" | "submitted"> = {};
  const lines = issueBody.split("\n");
  ...
  for (const line of lines) {
    ...
    // Determine state
    if (trimmedLine.includes("(submitted)")) {
      itemStates[itemName] = "submitted";
...
export function extractItemSections(issueBody: string): Record<string, string> {
  ...
  for (const line of lines) {
    ...
    // Match section headers like "### Myth", "### Promise", etc.
    ...
    if (trimmedLine.startsWith("- [") && currentSection) {
      const itemName = extractItemName(trimmedLine);
      if (itemName) {
        itemToSection[itemName] = currentSection;
      }
    }
  }

  return itemToSection;
}
