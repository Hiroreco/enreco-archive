...
function buildTimestampPingMessage(updates: TimestampUpdate[]): string {
    return updates
        .map((update) => {
            const pings = update.discordIds.map((id) => `<@${id}>`).join(", ");
            return `**\\[C${update.chapter}D${update.day}\\]** ⏱️ **${update.pov}** timestamps ready [here](<${update.link}>). Pinging ${pings}`;
        })
        .join("\n");
}
...
async function processTimestampUpdates(): Promise<void> {
    ...
    try {
        ...
        for (const [pov, link] of Object.entries(changedTimestamps)) {
            ...
            for (const [task, writer] of Object.entries(writerAssignments)) {
                if (new RegExp(`\\b${pov}\\b`, "i").test(task)) {
                    writers.add(writer);
                }
            }

            const writersArray = Array.from(writers);
            const discordIds = writersArray.map((w) => discordMappings[w] || w);
